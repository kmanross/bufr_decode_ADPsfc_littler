...
C
C   INPUT FILES:
C     unit  5  - size of Input BUFR file in bytes (ie from ls)
...
C        PRINT *,'ERROR IN FUNCTION STAT GETTING FILE STATS'
C        CALL EXIT(99)
C     ELSE
C        KBYTES = JSTAT(8)
C        PRINT *,'NUMBER OF BYTES IN INPUT BUFR FILE   = ',KBYTES
C     ENDIF
C
C  READ BYTE LENGTH OF BUFR FILE FROM STANDARD INPUT UNIT 5
C  --------------------------------------------------------

      READ(5,*,END=900,ERR=900) KBYTES
...
C           ibeg=ibeg+ilen
      ...
      stop
900   call bort('eof or err on std input - should be bytesize')
      end
